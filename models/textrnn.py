...
class TextRNN:
	def __init__(self, glove_word_matrix, response_length, cnn_ngrams, learning_rate, from_save = None):
		self._response_length = response_length
		self._glove_n_words = glove_word_matrix.shape[0]
		self._glove_n_dim = glove_word_matrix.shape[1]
		...
		with self._graph.as_default() as g:
			if from_save is None:
				...
				self._glove_matrix_holder = tf.placeholder(tf.float32, [self._glove_n_words, self._glove_n_dim], name = "glove_holder")
				
				glove_matrix = tf.Variable(self._glove_matrix_holder, trainable = False)

				embedded_query = tf.nn.embedding_lookup(glove_matrix, self._query)
				embedded_query = tf.reshape(embedded_query, [-1, 1, self._response_length, self._glove_n_dim])
				
				cnn_layer = tf.layers.conv2d(embedded_query, 
					filters = self._glove_n_dim, 
					kernel_size = [1, self._cnn_ngrams]
				)

				cnn_layer = tf.reshape(cnn_layer, [-1, self._response_length - cnn_ngrams + 1, self._glove_n_dim])

				rnn_input = tf.unstack(cnn_layer, axis = 1)
				lstm_fw_cell = tf.contrib.rnn.BasicLSTMCell(self._glove_n_dim, forget_bias = 1.0, activation = tf.tanh)
				lstm_bw_cell = tf.contrib.rnn.BasicLSTMCell(self._glove_n_dim, forget_bias = 1.0, activation = tf.tanh)
				...
				self._sess.run(init, feed_dict = {
					self._glove_matrix_holder: glove_word_matrix
				})
				...
				self._glove_matrix_holder = g.get_tensor_by_name("glove_holder:0")
